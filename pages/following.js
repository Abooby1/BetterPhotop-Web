...
<span id="detailInfo" style="color: var(--fontColor)">Users Followed</span>
</div>
<div class="followHolder"></div>
`;


pages.following = function () {
    ...
    if (code == 200) {
      ...
      if (data.length < 25) {
        ...
        if (followHolder.childElementCount > 0) {
          createTooltip(followHolder, "That's all of the users followed...");
        } else {
          createTooltip(followHolder, "Well... No one is followed...");
        }
      }
      loading = false;
    } else {
  ...
  getFollow("user/following?userid=" + getID, true);
  ...
  tempListen(document, "scroll", function() {
    if (!reachedMax && !loading) {
      if (window.innerHeight + window.scrollY >= followHolder.offsetHeight - 500) {
        getFollow("user/following?userid=" + getID + "&before=" + followHolder.lastChild.getAttribute("time"));
      }
    }
  });
}